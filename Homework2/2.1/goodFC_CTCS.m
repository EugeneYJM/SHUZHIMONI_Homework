...
while count<135
    ...
    if mod(count,5)==0
        ...
        title(['1维Bugger’s非线性平流方程2.5.10b Time:' num2str(count*dt),'s']);
        xlabel('x')
        ylabel('T')
        pause(0.05);
        frame = getframe(gcf);
        ...
        if count==5
            imwrite(imind,cm,'assignment2_1_Bugger’s_eq_2_5_10_b.gif','gif', 'Loopcount',inf);
        else
            imwrite(imind,cm,'assignment2_1_Bugger’s_eq_2_5_10_b.gif','gif','WriteMode','append');
        end
    end %画图部分
end
